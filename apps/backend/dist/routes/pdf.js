...
async function buildPdf(reportId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (customer?.phone_number) {
            doc.text(`Tel: ${customer.phone_number}`, M, y);
            y += 13;
        }
        const rx = M + CW / 2;
        let ry = 126;
        ...
        if (assignment?.scheduled_at) {
            doc.fillColor(DARK).font('Helvetica').fontSize(10)
                .text(`Termin: ${fmtDate(assignment.scheduled_at)}`, rx, ry);
            ry += 13;
        }
        y = Math.max(y, ry) + 18;
        // ── Table header ───────────────────────────────────────────────────────────
...
router.get('/:reportId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="helferchen-rechnung-${req.params.reportId}.pdf"`);
        res.send(pdfBuffer);
    }
    catch (err) {
