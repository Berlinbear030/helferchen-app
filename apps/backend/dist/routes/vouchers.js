    ...
    try {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
                const exists = await (0, pool_1.query)('SELECT id FROM vouchers WHERE code = ?', [code]);
                if (exists.rows.length === 0)
                    break;
                code = generateCode();
            }
            const id = (0, crypto_1.randomUUID)();
            await (0, pool_1.query)(`INSERT INTO vouchers (id, code, label, discount_type, discount_value, max_uses, expires_at, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, code, label, discount_type, discount_value, max_uses || null, expires_at || null, notes || null]);
            created.push(code);
        }
...
router.post('/validate', authenticate, async (req, res) => {
    ...
    try {
        const result = await (0, pool_1.query)('SELECT * FROM vouchers WHERE code = ? AND active = TRUE', [code.toUpperCase().trim()]);
...
router.post('/apply', authenticate, async (req, res) => {
    ...
    if (!code || !report_id)
        return res.status(400).json({ message: 'code und report_id sind erforderlich' });
    try {
        const vResult = await (0, pool_1.query)('SELECT * FROM vouchers WHERE code = ? AND active = TRUE', [code.toUpperCase().trim()]);
        ...
        const existing = await (0, pool_1.query)('SELECT id, voucher_id FROM voucher_usages WHERE report_id = ?', [report_id]);
        if (existing.rows.length > 0) {
            const old = existing.rows[0];
            await (0, pool_1.query)('DELETE FROM voucher_usages WHERE report_id = ?', [report_id]);
            await (0, pool_1.query)('UPDATE vouchers SET used_count = GREATEST(0, used_count - 1) WHERE id = ?', [old.voucher_id]);
            await (0, pool_1.query)('UPDATE reports SET voucher_code = NULL, voucher_label = NULL, voucher_discount_amount = NULL WHERE id = ?', [report_id]);
        }
        const usageId = (0, crypto_1.randomUUID)();
        await (0, pool_1.query)('INSERT INTO voucher_usages (id, voucher_id, report_id, discount_amount) VALUES (?, ?, ?, ?)', [usageId, v.id, report_id, discount_amount]);
        await (0, pool_1.query)('UPDATE vouchers SET used_count = used_count + 1 WHERE id = ?', [v.id]);
        await (0, pool_1.query)('UPDATE reports SET voucher_code = ?, voucher_label = ?, voucher_discount_amount = ? WHERE id = ?', [v.code, v.label, discount_amount, report_id]);
...
router.delete('/apply/:reportId', authenticate, requireAdmin, async (req, res) => {
    try {
        const existing = await (0, pool_1.query)('SELECT id, voucher_id FROM voucher_usages WHERE report_id = ?', [req.params.reportId]);
        if (existing.rows.length > 0) {
            const old = existing.rows[0];
            await (0, pool_1.query)('DELETE FROM voucher_usages WHERE report_id = ?', [req.params.reportId]);
            await (0, pool_1.query)('UPDATE vouchers SET used_count = GREATEST(0, used_count - 1) WHERE id = ?', [old.voucher_id]);
        }
        await (0, pool_1.query)('UPDATE reports SET voucher_code = NULL, voucher_label = NULL, voucher_discount_amount = NULL WHERE id = ?', [req.params.reportId]);
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.patch('/:id', authenticate, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await (0, pool_1.query)(`UPDATE vouchers SET ${sets.join(', ')} WHERE id = ?`, vals);
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.delete('/:id', authenticate, requireAdmin, async (req, res) => {
    try {
        await (0, pool_1.query)('DELETE FROM vouchers WHERE id = ?', [req.params.id]);
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ message: err.message });
